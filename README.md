...
- 修复已读状态不同步问题：加载会话和查看已读明细时都会显式刷新当前用户已读游标，避免对方已读后发送方仍看到 0；同时修复聊天页 `api is not defined`。
...
- 聊天输入框支持直接粘贴剪贴板图片，自动作为待发送附件上传，并复用 1–100% 上传进度。
...
- 管理员端新增“操作日志”页面，可按关键词、用户 ID、操作类型查询登录、退出、发消息、上传附件、后台用户管理、注册链接和站点设置等审计记录。
- 用户管理列表新增最后上线时间与在线/离线状态；登录、会话刷新、进入聊天初始化和发送消息都会更新用户最后上线时间。
- 部署工作流新增 D1 活动日志结构检查，会自动补齐 `users.last_seen_at` 和 `operation_logs` 表及索引。
